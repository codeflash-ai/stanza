...
class MultiHeadAttention(nn.Module):
    ...
    def split_qkv_packed(self, inp, qk_inp=None):
        # Preallocate all repeats instead of using .repeat + .view for each call
        n_head = self.n_head
        batch_len = inp.size(0)
        v_inp_repeated = inp.expand(n_head, batch_len, inp.size(-1))
        if qk_inp is None:
            qk_inp_repeated = v_inp_repeated
        else:
            qk_inp_repeated = qk_inp.expand(n_head, batch_len, qk_inp.size(-1))
        if not self.partitioned:
            ...
            q_s_content = torch.bmm(qk_inp_repeated[:, :, :d_content], self.w_qs1)
            q_s_positional = torch.bmm(qk_inp_repeated[:, :, d_content:], self.w_qs2)
            q_s = torch.cat([q_s_content, q_s_positional], -1)

            k_s_content = torch.bmm(qk_inp_repeated[:, :, :d_content], self.w_ks1)
            k_s_positional = torch.bmm(qk_inp_repeated[:, :, d_content:], self.w_ks2)
            k_s = torch.cat([k_s_content, k_s_positional], -1)

            v_s_content = torch.bmm(v_inp_repeated[:, :, :d_content], self.w_vs1)
            v_s_positional = torch.bmm(v_inp_repeated[:, :, d_content:], self.w_vs2)
            v_s = torch.cat([v_s_content, v_s_positional], -1)
        return (q_s, k_s, v_s)
    ...
    def pad_and_rearrange(self, q_s, k_s, v_s, batch_idxs):
        ...
        # Use torch.zeros directly instead of new_zeros for potential speedup
        device = q_s.device
        dtype = q_s.dtype
        q_padded = torch.zeros((n_head, mb_size, len_padded, d_k), dtype=dtype, device=device)
        k_padded = torch.zeros((n_head, mb_size, len_padded, d_k), dtype=dtype, device=device)
        v_padded = torch.zeros((n_head, mb_size, len_padded, d_v), dtype=dtype, device=device)
        invalid_mask = torch.ones((mb_size, len_padded), dtype=DTYPE, device=device)
        # Avoid enumerate/zip+slice for loop, optimize boundary handling
        ...
        # Expand in a single op
        exp_invalid = invalid_mask.unsqueeze(1).expand(mb_size, len_padded, len_padded)
        exp_invalid = exp_invalid.repeat(n_head, 1, 1)
        output_mask = (~invalid_mask).repeat(n_head, 1)
        return (q_padded.view(-1, len_padded, d_k), k_padded.view(-1, len_padded, d_k), v_padded.view(-1, len_padded, d_v), exp_invalid, output_mask)

    def combine_v(self, outputs):
        n_head = self.n_head
        # Avoid transpose/view patterns that break contiguity, use reshape to ensure effective memory access
        outputs = outputs.view(n_head, -1, self.d_v)
        if not self.partitioned:
            outputs = torch.transpose(outputs, 0, 1).reshape(-1, n_head * self.d_v)
            outputs = self.proj(outputs)
        else:
            d_v1 = self.d_v // 2
            outputs1 = outputs[:, :, :d_v1]
            outputs2 = outputs[:, :, d_v1:]
            outputs1 = torch.transpose(outputs1, 0, 1).reshape(-1, n_head * d_v1)
            outputs2 = torch.transpose(outputs2, 0, 1).reshape(-1, n_head * d_v1)
...
class LabelAttentionModule(nn.Module):
    ...
    @codeflash_capture(function_name='LabelAttentionModule.__init__', tmp_dir_path='/tmp/codeflash_o7juese7/test_return_values', tests_root='/home/ubuntu/work/repo/stanza/tests', is_fto=True)
    ...
    def forward(self, word_embeddings, tagged_word_lists):
        ...
        sentence_lengths = [sentence.shape[0] for sentence in word_embeddings]
        batch_size = len(word_embeddings)
        batch_idxs = np.repeat(np.arange(batch_size), sentence_lengths)
        batch_idxs = BatchIndices(batch_idxs, word_embeddings[0].device)
        new_word_embeddings = torch.cat(word_embeddings, dim=0)
        ...
        final_labeled_representations = list(torch.split(labeled_representations, sentence_lengths))
