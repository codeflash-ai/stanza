...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, n_head, d_model, d_k, d_v, residual_dropout=0.1, attention_dropout=0.1, d_positional=None):
        ...
        if self.partitioned:
            ...
            # Preallocate parameter tensors (do not use nn.ParameterList for strict behavior preservation)
    ...
    def split_qkv_packed(self, inp, qk_inp=None):
        """
        Split and compute Q, K, V projections for multi-head attention.
        Optimizations:
        - Use expand rather than repeat when possible to avoid extra copies.
        - Use view and expand to avoid unnecessary memory overhead.
        - Merge cat+bmm pattern for partitioned case.
        """

        batch_size = inp.size(0)
        seq_len = inp.size(1) if inp.dim() > 1 else 1  # handle 1D or 2D

        # Use expand for efficiency where possible (read-only, avoids data copy)
        # For not partitioned case
        if qk_inp is None:
            qk_inp = inp

        # Prepare for batched matmul, shape: (n_head, batch, d_model)
        v_inp_exp = inp.expand(self.n_head, *inp.shape)
        qk_inp_exp = qk_inp.expand(self.n_head, *qk_inp.shape)

        if not self.partitioned:
            # bmm each head in parallel: (n_head, batch, d_model) x (n_head, d_model, d_k) -> (n_head, batch, d_k)
            q_s = torch.bmm(qk_inp_exp, self.w_qs)
            k_s = torch.bmm(qk_inp_exp, self.w_ks)
            v_s = torch.bmm(v_inp_exp, self.w_vs)
        else:
            # Slice upfront for both heads to avoid multiple slices (minimize repeated slicing)
            qk_content = qk_inp_exp[:, :, :self.d_content]
            qk_pos    = qk_inp_exp[:, :, self.d_content:]

            v_content = v_inp_exp[:, :, :self.d_content]
            v_pos     = v_inp_exp[:, :, self.d_content:]

            # Batched bmm for heads
            q_s1 = torch.bmm(qk_content, self.w_qs1)
            q_s2 = torch.bmm(qk_pos,    self.w_qs2)
            q_s = torch.cat([q_s1, q_s2], -1)

            k_s1 = torch.bmm(qk_content, self.w_ks1)
            k_s2 = torch.bmm(qk_pos,    self.w_ks2)
            k_s = torch.cat([k_s1, k_s2], -1)

            v_s1 = torch.bmm(v_content, self.w_vs1)
            v_s2 = torch.bmm(v_pos,    self.w_vs2)
            v_s = torch.cat([v_s1, v_s2], -1)
