...
class LabelAttention(nn.Module):
    ...
    def __init__(
        self, d_model, d_k, d_v, d_l, d_proj, combine_as_self, use_resdrop=True,
        q_as_matrix=False, residual_dropout=0.1, attention_dropout=0.1, d_positional=None
    ):
        ...
        self.partitioned = bool(d_positional)
        if self.partitioned:
            if d_model <= d_positional:
                raise ValueError(
                    'Unable to build LabelAttention.  d_model %d <= d_positional %d' % (d_model, d_positional)
                )
            self.d_content = d_model - d_positional
            self.d_positional = d_positional

            # Pre-initialize parameter shapes for speed  
            if self.q_as_matrix:
                self.w_qs1 = nn.Parameter(
                    torch.empty(self.d_l, self.d_content, d_k // 2)
                )
            else:
                self.w_qs1 = nn.Parameter(
                    torch.empty(self.d_l, d_k // 2)
                )
            self.w_ks1 = nn.Parameter(
                torch.empty(self.d_l, self.d_content, d_k // 2)
            )
            self.w_vs1 = nn.Parameter(
                torch.empty(self.d_l, self.d_content, d_v // 2)
            )
            if self.q_as_matrix:
                self.w_qs2 = nn.Parameter(
                    torch.empty(self.d_l, self.d_positional, d_k // 2)
                )
            else:
                self.w_qs2 = nn.Parameter(
                    torch.empty(self.d_l, d_k // 2)
            ...
            self.w_vs2 = nn.Parameter(
                torch.empty(self.d_l, self.d_positional, d_v // 2)
            )
            # Batch initialize
            init.xavier_normal_(self.w_qs1)
            init.xavier_normal_(self.w_ks1)
            ...
            if self.q_as_matrix:
                self.w_qs = nn.Parameter(
                    torch.empty(self.d_l, d_model, d_k)
                )
            else:
                self.w_qs = nn.Parameter(
                    torch.empty(self.d_l, d_k)
                )
            self.w_ks = nn.Parameter(
                torch.empty(self.d_l, d_model, d_k)
            )
            self.w_vs = nn.Parameter(
                torch.empty(self.d_l, d_model, d_v)
            )
            init.xavier_normal_(self.w_qs)
            init.xavier_normal_(self.w_ks)
            init.xavier_normal_(self.w_vs)
    ...
    def split_qkv_packed(self, inp, k_inp=None):
        len_inp = inp.size(0)
        # Precompute repeated tensors efficiently
        v_inp_expanded = inp.expand(self.d_l, -1, -1)
        if k_inp is None:
            k_inp_expanded = v_inp_expanded
        else:
            k_inp_expanded = k_inp.expand(self.d_l, -1, -1)

        if not self.partitioned:
            if self.q_as_matrix:
                q_s = torch.bmm(k_inp_expanded, self.w_qs)
            else:
                q_s = self.w_qs.unsqueeze(1)
            k_s = torch.bmm(k_inp_expanded, self.w_ks)
            v_s = torch.bmm(v_inp_expanded, self.w_vs)
        else:
            if self.q_as_matrix:
                k_content = k_inp_expanded[:, :, :self.d_content]
                k_pos = k_inp_expanded[:, :, self.d_content:]
                q_s_content = torch.bmm(k_content, self.w_qs1)
                q_s_pos = torch.bmm(k_pos, self.w_qs2)
                q_s = torch.cat([q_s_content, q_s_pos], -1)
            else:
                q_s1 = self.w_qs1.unsqueeze(1)
                q_s2 = self.w_qs2.unsqueeze(1)
                q_s = torch.cat([q_s1, q_s2], -1)
            v_content = v_inp_expanded[:, :, :self.d_content]
            v_pos = v_inp_expanded[:, :, self.d_content:]
            k_content = k_inp_expanded[:, :, :self.d_content]
            k_pos = k_inp_expanded[:, :, self.d_content:]
            k_s = torch.cat([
                torch.bmm(k_content, self.w_ks1),
                torch.bmm(k_pos, self.w_ks2)], -1)
            v_s = torch.cat([
                torch.bmm(v_content, self.w_vs1),
                torch.bmm(v_pos, self.w_vs2)], -1)
        return (q_s, k_s, v_s)
