...
def unmap_with_copy(indices, src_tokens, vocab):
    ...
    """
    result = []
    id2word = vocab.id2word  # Localize attribute for faster access
    append_result = result.append  # Localize method for faster access

    for ind, tokens in zip(indices, src_tokens):
        words = [id2word[idx] if idx >= 0 else tokens[-idx - 1] for idx in ind]
        append_result(words)
    return result
