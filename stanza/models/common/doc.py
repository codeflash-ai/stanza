...
class Token(StanzaObject):
    ...
    def to_dict(self, fields=DEFAULT_OUTPUT_FIELDS):
        ...
        for word in self.words:
            ...
            if id_len == 1:
                if NER in fields and getattr(self, NER) is not None: # propagate NER label to Word if it is a single-word token
                    word_dict[NER] = getattr(self, NER)
                if MULTI_NER in fields and getattr(self, MULTI_NER) is not None: # propagate MULTI_NER label to Word if it is a single-word token
                    word_dict[MULTI_NER] = getattr(self, MULTI_NER)
                if MISC in fields:
                    spaces_after = self.spaces_after
                    spaces_before = self.spaces_before
                    has_misc = word_dict.get(MISC)
                    if spaces_after is not None and spaces_after != ' ':
                        space_misc = space_after_to_misc(spaces_after)
                        if has_misc:
                            word_dict[MISC] = has_misc + "|" + space_misc
                        else:
